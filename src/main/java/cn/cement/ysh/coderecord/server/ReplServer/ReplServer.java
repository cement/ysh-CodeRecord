package cn.cement.ysh.coderecord.server.ReplServer;
/**
 * default package
 */
...
//			}
//		}
        HttpServer server = HttpServer.create(new InetSocketAddress(39193), 0);
	...
	static class CodeHandler implements HttpHandler {
		...
		public void handle(HttpExchange exchange) throws IOException {
			...
			if ("GET".equals(method)) {
				...
				if (!body.startsWith("code=")) {
					...
					String code = URLDecoder.decode(body.substring(5), StandardCharsets.UTF_8.name());
		...
		void sendData(HttpExchange exchange, String s) throws IOException {
			String origin = exchange.getRequestHeaders().getOrDefault("Origin", Arrays.asList("www.cement.cn")).get(0);
